...
public class HibernateDAO {
    ...
    public <T> List<T> listByPageByCriteriaOrderBy(Class<T> clazz,
        ...
        criteria.add(criterion);
        if(isAsc){
            criteria.addOrder(Order.asc(orderByFlie));
        }else{
            criteria.addOrder(Order.desc(orderByFlie));
        }
        criteria.setFirstResult(pageCurrent * pageSize);
        criteria.setMaxResults(pageSize);
        @SuppressWarnings("unchecked")
     ...
     */
    @SuppressWarnings("unchecked")
    public <T> List<T> listFirstByCriteria(Class<T> clazz, Criterion criterion,
                                           boolean isCached) {
        Session session = sessionFactory.getCurrentSession();
        Criteria criteria = session.createCriteria(clazz);
        criteria.setCacheable(isCached);
        criteria.add(criterion);
        criteria.setFirstResult(0);  //设定查询返回结果的第一行数据的位置
        criteria.setMaxResults(1);   //限定查询返回数据的行数
        List<T> result = criteria.list();
        return result;
    }
    ...
    public int CountBySQL(String listSqlString) {
        ...
        if (null != result) {
            ...
            String bd = map.get("SUM").toString();
